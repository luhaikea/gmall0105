...
解决：在点击结算时生成一个交易码（tradeCode），分别在服务器（redis）和结算页面存放，在结算页面点击提交订单时，
在控制器会进行服务器和结算页面的交易码的比较检查，交易码校验之后会销毁
交易码是根据memberId生成的
*/
        ...
        if (success.equals("success")) {
            ...
            for (OmsCartItem omsCartItem : omsCartItems) {
                if (omsCartItem.getIsChecked().equals("1")) {
                    ...
                    omsOrderItem.setOrderSn(outTradeNo);// 外部订单号，用来和其他系统进行交互，防止重复,就是在支付宝中标识此订单
            ...
            //为了安全不应该直接传金额  应该在拦截器中用memberId在数据库中查找并加入
